...
struct MultiSigDeleGatorStorage {
    ...
    mapping(address => bool) usesERC1271; // Tracks which signers use ERC1271 (set at addition)
}
...
contract MultiSigDeleGator is DeleGatorCore {
     ...
     * @dev There is practically no chance of composing a signature whose first 20 bytes equal an address of a contract signer,
     * @dev as doing so is as impractical as deriving the private key of a signer. Complexity of ~2^160.
     * @param _hash The data signed
     * @param _signatures Concatenated signatures from threshold signers, sorted by signer address
     * @return The EIP1271 magic value if the signature is valid, otherwise it reverts with ERC1271Lib.SIG_VALIDATION_FAILED
     */
    ...
    function _supportsERC1271(address _contract) internal view returns (bool) {
        ...
        (bool success, bytes memory result) = _contract.staticcall{ gas: 30_000 }(data);
